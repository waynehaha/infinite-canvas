...
const AIHUB_PREVIOUS_DEFAULT_MODELS = [...AIHUB_DEFAULT_MODELS, "gpt-5-5", "gpt-5-3", "gpt-5-3-mini", "gpt-5-2"];
...
export function normalizeLocalChannels(config: Partial<AiConfig>) {
    ...
    const normalized = channels.map((channel, index) => {
        ...
        const managedAIHub = channel.id === "aihub" && [AIHUB_LEGACY_DEFAULT_MODELS, AIHUB_PREVIOUS_DEFAULT_MODELS].some((defaults) => sameModelSet(models, defaults));
        return {
