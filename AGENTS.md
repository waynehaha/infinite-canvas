...
- 凡涉及 Web 页面、网页交互或界面改动，交付前必须尽量启动并验证一个可访问的网页服务。
- 如果网页服务已启动，最终回复必须附上可点击的访问链接，例如 `http://127.0.0.1:3000/`。
- 如果无法启动网页服务，必须在最终回复中说明具体原因，并说明已采用的替代验证方式和剩余风险。
- 不要为了提供链接而启动与本次项目无关的服务；链接必须对应当前项目和本次改动。
